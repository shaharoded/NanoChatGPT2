...
Fine-tuning the nanoGPT model on ~88MB of QA data with the current configurations took ~3 hours (CPU), achieving `val loss ~1.72`. While the model could generate responses, they were often incorrect, showcasing the limitations of a small architecture and limited data.
