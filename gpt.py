...
class GPT(nn.Module):
    ...
    def generate_text(self, context, max_new_tokens=200, device=None, temperature=1.0, top_k=None):
        '''
        Generate text using 'prefix' as context (not including prefix).
        Uses self.generate and the selected tokenizer to do so.
        
        Args:
            context (str): Text used as prefix to the model generation.
            max_new_tokens (int): Maximum number of tokens to generate (to limit answer size).
            device (str): CPU or GPU, based on what set on the training module.
            temperature (float): The temperature value for controlling the randomness of the generation. 
                                Lower values (e.g., 0.7) make the output more deterministic, while higher values 
                                (e.g., 1.2) make it more random.
            top_k (int, optional): The number of highest probability tokens to keep for sampling. This helps 
        ...
        if context is None:
            # Starting token (adjust as necessary)
            context = torch.zeros((1, 1), dtype=torch.long, device=device)
        elif isinstance(context, str):
            # Encode string context to tokens
            input_tokens = encode(context)
            context = torch.tensor(input_tokens, dtype=torch.long).unsqueeze(0).to(device)
        else:
